...
export function NextProjectLink({ href, title }: { href: string; title: string }) {
  return (
    <section className="flex min-h-[88px] w-full items-center justify-between gap-4 border-t border-vinus-ink/10 px-[var(--space-edge)] md:min-h-[72px] md:gap-8">
      <span className="label-sm shrink-0 font-medium text-vinus-ink/45">Next Project</span>
      <Link href={href} className="group flex min-w-0 max-w-full items-center gap-4 md:gap-6">
        <span
          className="heading-detail-next min-w-0 truncate font-normal transition-opacity duration-200 group-hover:opacity-60"
        >
          {title}
        </span>
