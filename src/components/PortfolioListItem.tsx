...
export function PortfolioListItem({ title, subtitle, category, image, href, index = 0 }: PortfolioListItemProps) {
  ...
  return (
    <motion.a
      href={href}
      initial={reduceMotion ? false : { opacity: 0, y: 124, scale: 0.985, clipPath: "inset(22% 0 0 0)" }}
      ...
      transition={{ duration: reduceMotion ? 0 : 1, delay: reduceMotion ? 0 : index * 0.09, ease: [0.16, 1, 0.3, 1] }}
    ...
    >
      ...
      <div className="min-w-0 transition-transform duration-300 ease-[cubic-bezier(0.16,1,0.3,1)] group-hover:translate-x-1">
        <h2 className="type-heading font-normal transition-colors duration-300 group-hover:text-vinus-ink/80">{title}</h2>
