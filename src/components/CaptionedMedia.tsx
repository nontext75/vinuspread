...
type CaptionedMediaProps = {
  ...
  meta?: string;
  sizes?: string;
  ratio?: "square" | "landscape" | "contact";
  className?: string;
};
...
export function CaptionedMedia({ src, alt, title, meta, sizes = "100vw", ratio = "landscape", className }: CaptionedMediaProps) {
  ...
  return (
    <div
      ref={ref}
      className={cn(
        "captioned-media relative overflow-hidden bg-vinus-wash-soft",
        ratio === "square" && "captioned-media--square",
        ratio === "landscape" && "captioned-media--landscape",
        ratio === "contact" && "captioned-media--contact",
        className,
      )}
    >
      ...
      <div className="absolute inset-x-0 bottom-0 flex items-end justify-between gap-4 bg-gradient-to-t from-black/55 to-transparent px-6 pb-12 pt-16 text-white">
        <p className="body-md font-medium">{title}</p>
        {meta && <p className="body-md text-right">{meta}</p>}
      </div>
    </div>
  );
}
